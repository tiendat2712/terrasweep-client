...
export const BuyerHeader: React.FC<BuyerHeaderProps> = ({
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      {/* ========================================================================= */}
      <header className="w-full bg-white/95 backdrop-blur-md border-y border-zinc-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 sm:h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center shrink-0">
            ...
            >
              ...
              <span className="text-[26px] sm:text-[28px] md:text-[30px] font-black tracking-[-0.03em] text-[#0C0C0C] font-sans group-hover:opacity-85 transition-opacity">
                TerraSweep
              </span>
            </button>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-8 lg:gap-11 text-sm font-medium text-zinc-600">
            <button
              onClick={() => {
                onSearch('');
                handleNavClick();
              }}
              className="hover:text-[#0C0C0C] transition-colors cursor-pointer py-1"
            >
              {t('nav.home')}
            </button>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
            ...
            <div className="flex items-center rounded-full bg-zinc-100 p-0.5 text-[10px] font-mono font-bold border border-zinc-200 select-none">
              <button
            ...
            <button
              onClick={onOpenCart}
              className="w-10 h-10 sm:w-11 sm:h-11 rounded-full border border-zinc-300 hover:border-[#0C0C0C] bg-white text-zinc-800 hover:text-black flex items-center justify-center relative shadow-2xs hover:scale-105 active:scale-95 transition-all cursor-pointer group"
              title={t('cart.title')}
              aria-label="Shopping Cart"
            >
              <ShoppingCart className="w-4 h-4 sm:w-4.5 sm:h-4.5 transition-transform group-hover:scale-110" />
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[19px] h-[19px] px-1 rounded-full bg-[#0C0C0C] text-white text-[9px] font-mono font-bold flex items-center justify-center shadow-xs animate-in zoom-in-50">
                  {cartItemCount}
                </span>
              )}
            </button>
            ...
            {currentUser && currentUser.role !== 'customer' ? (
              ...
              <button
                ...
                className="h-10 sm:h-11 pl-4 pr-1.5 sm:pr-2 rounded-full border border-zinc-300 hover:border-black bg-white hover:bg-zinc-50/50 flex items-center gap-2.5 transition-all shadow-2xs group cursor-pointer active:scale-95"
                title={t('nav.login')}
              >
                <span className="text-xs sm:text-sm font-semibold text-[#0C0C0C] tracking-tight">
                  {t('nav.login')}
                </span>
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#0C0C0C] text-white flex items-center justify-center group-hover:scale-105 group-hover:bg-zinc-800 transition-all shadow-2xs">
            ...
            <button
              ...
              className="md:hidden w-10 h-10 rounded-full border border-zinc-300 bg-white hover:border-black flex items-center justify-center text-zinc-800 transition-all cursor-pointer"
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden border-t border-zinc-200 bg-white px-6 py-5 space-y-4 animate-in slide-in-from-top-2 duration-200">
            <div className="flex items-center justify-between pb-3 border-b border-zinc-100">
              <span className="text-xs font-mono font-bold uppercase text-zinc-400">TerraSweep Menu</span>
              <button
                onClick={() => {
                  setIsMobileMenuOpen(false);
                  onOpenTracking();
            ...
            <div className="flex flex-col space-y-3 text-sm font-semibold text-zinc-800">
              <button
                onClick={() => {
                  onSearch('');
                  handleNavClick();
                }}
                className="text-left py-1 hover:text-black cursor-pointer"
              >
                {t('nav.home')}
              </button>
              <button
      ...
      <section
        ...
        className="w-full bg-[#FAFAFA] border-b border-zinc-200/80 py-6 sm:py-8 px-4 sm:px-6"
      >
        <div className="max-w-3xl mx-auto flex flex-col items-center gap-3.5">
          
          {/* Master Search Input Bar */}
          <div className="w-full flex items-center rounded-full bg-white border border-zinc-300 hover:border-zinc-400 focus-within:border-[#0C0C0C] focus-within:shadow-md shadow-2xs pl-4 sm:pl-5 pr-1.5 sm:pr-2 py-1.5 sm:py-2 transition-all duration-200">
            <Search className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-400 mr-2.5 sm:mr-3 shrink-0" />
            ...
            <input
              ...
              className="w-full bg-transparent text-xs sm:text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none font-medium"
            />
            ...
            {/* Deep Black Solid Search Action Button */}
            <button
              onClick={handleTriggerSearch}
              className="rounded-full bg-[#0C0C0C] hover:bg-zinc-800 text-white px-5 sm:px-6 py-2 sm:py-2.5 text-xs font-bold flex items-center gap-2 transition-all shrink-0 shadow-2xs cursor-pointer active:scale-95"
            >
